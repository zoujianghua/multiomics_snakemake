...
# -*- coding: utf-8 -*-
"""
【HSI patch pipeline - 3D CNN 训练】

Patch-level 3D CNN:
- 输入: --index-tsv patch_index_phase_core_seed42.tsv
- 如果可用，优先使用 scripts.hsi.hsi_patch_dataset.HSIPatchDataset(mode="3d")
- 否则使用脚本内置 Fallback 数据集:
    x shape: [1, bands, H, W] -> [B, 1, bands, H, W]
...
# 尝试优先使用你已有的 HSIPatchDataset（假定支持 mode="3d"）
try:
    # 尝试多种导入方式
    try:
        from hsi_patch_dataset import HSIPatchDataset as _ExtHSIPatchDataset, get_train_transform_3d
    except ImportError:
...
class HSIPatchDataset3DFallback(Dataset):
    ...
    def __init__(self, index_tsv, split="train", bands=None):
        ...
        if "patch_npz" in df.columns and df["patch_npz"].notna().any():
            ...
            print(f"[HSIPatchDataset3DFallback] 使用 patch_npz 模式（{len(df)} patches）")
        elif "cube_npz" in df.columns and "y0" in df.columns and "x0" in df.columns and "size" in df.columns:
            self.mode_type = "cube"
            print(f"[HSIPatchDataset3DFallback] 使用 cube 模式（兼容旧格式，{len(df)} patches）")
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.mode_type == "patch_npz":
            ...
            # 3D CNN 期望 [B, C, D, H, W]，这里 C=1, D=bands
...
class HSIPatchNet3D(nn.Module):
    """
    升级版 3D CNN（轻量 ResNet 风格）：
    输入 x: [B, 1, D, H, W]，D 为光谱维度
    - 先用 1×1×k 的 3D conv 做光谱维降维
    - 堆 2-3 个轻量 3D residual block
    - GAP 后接 FC 分类头
    """

    def __init__(self, in_channels, n_classes, embedding_dim=128):
        super().__init__()
        # 光谱维降维（1×1×k 的 3D conv）
        self.conv_reduce = nn.Conv3d(in_channels, 16, kernel_size=(1, 1, 3), padding=(0, 0, 1))
        self.bn_reduce = nn.BatchNorm3d(16)

        # 残差块
        self.res_block1 = ResidualBlock3D(16, 32, stride=1)
        self.res_block2 = ResidualBlock3D(32, 64, stride=2)
        self.res_block3 = ResidualBlock3D(64, 64, stride=1)
        
        # 全局平均池化
        self.gap = nn.AdaptiveAvgPool3d(1)
        
        # 特征提取层（用于 forward_features）
        self.feature_fc = nn.Linear(64, embedding_dim)
...
def get_datasets(index_tsv, augment=True, augment_noise_std=0.0, seed=42):
    ...
    if augment and get_train_transform_3d is not None:
        train_transform = get_train_transform_3d(noise_std=augment_noise_std, seed=seed)
    try:
        train_ds = _ExtHSIPatchDataset(index_tsv, split="train", mode="3d", transform=train_transform)
        test_ds = _ExtHSIPatchDataset(index_tsv, split="test", mode="3d")
        print(f"[3dcnn] 使用 HSIPatchDataset (mode=3d, patch_cubes 模式, train_augment={augment})")
...
def export_patch_embeddings(model, index_tsv, out_tsv, batch_size, num_workers, pin_memory, device, use_amp=False):
    ...
    print(f"[3dcnn] 导出 {len(output_df)} 个 patch 的 embedding（维度 {emb_dim}）到 {out_tsv}")
...
def main():
    ...
    ap.add_argument("--outdir", required=True,
                    help="results/hsi/patch/3dcnn_phase_core")
    ap.add_argument("--batch-size", type=int, default=8)
    ap.add_argument("--epochs", type=int, default=200)
    ap.add_argument("--weight-decay", type=float, default=5e-4,
                    help="Adam 优化器的 L2 正则系数，默认 1e-4")
    ...
    print(f"[3dcnn] device: {device}, amp={use_amp}, gradient_accumulation_steps={accum_steps}")
    ...
    print(f"[3dcnn] n_train={len(train_ds)}, n_test={len(test_ds)}, n_classes={n_classes}")
    ...
    print(f"[3dcnn] DataLoader num_workers={num_workers}, pin_memory={pin_memory}")

    # 取一个 batch 确定输入维度
    example_x, _ = next(iter(train_loader))  # [B, C, D, H, W] 或 [B, 1, D, H, W]
    if example_x.ndim == 4:
        # [B, 1, D, H, W] 情况
        example_x = example_x
    elif example_x.ndim == 5:
        # 正常 5D
        pass
    else:
        raise RuntimeError(f"unexpected input shape: {example_x.shape}")

    in_channels = example_x.shape[1]

    model = HSIPatchNet3D(in_channels=in_channels, n_classes=n_classes).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"[3dcnn][epoch {epoch}] time={epoch_time:.1f}s, "
        ...
        if epoch >= args.min_epochs:
            ...
            if no_improve_epochs >= args.early_stopping_patience:
                print(
                    f"[3dcnn] early stopping at epoch {epoch} "
    ...
    try:
        ...
        plt.close(fig)
        print(f"[3dcnn] train_val_curves -> {outdir / 'train_val_curves.png'}")
    except Exception as e:
        print(f"[3dcnn] 未绘制 train_val_curves: {e}")

    print(f"[3dcnn] best_model -> {model_path}")
    print(f"[3dcnn] metrics -> {metrics_path}")
    
    # 导出 patch embeddings
    emb_out = outdir / "patch_embeddings_3d.tsv"
    ...
    print(f"[3dcnn] patch embeddings -> {emb_out}")
